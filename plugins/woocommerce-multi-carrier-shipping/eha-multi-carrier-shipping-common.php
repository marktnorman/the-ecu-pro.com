...
if ( ! defined( 'ABSPATH' ) ) {
	...
	if (!function_exists('wf_plugin_override')){
		add_action( 'plugins_loaded', 'wf_plugin_override', 99 );
		...
		class eha_multi_carrier_shipping_setup {
			...
			function woocommerce_cart_shipping_packages($shipping)
			{
				if(isset($_POST['post_data']))
				{
					...
					if( isset($str['ship_to_different_address']) )
					{
						if( isset($str['ph_shipping_is_residential']) )
						{
						...
						if( isset($str['ph_billing_is_residential']) )
						{
							foreach($shipping as $key=>$val)
							{
								$shipping[$key]['destination']['is_residential'] = true;
			...
			public function wf_add_checkout_page_field($fields)
			{
				...
				foreach($fields as $key=>$val)
				{
					if( $key=='billing_first_name' )
					{
						...
						$out_f[$key] = $val;
						$out_f['ph_billing_is_residential'] = array(
							'type' 		=> 'checkbox',
						...
						$out_f[$key] = $val;
						$out_f['ph_shipping_is_residential'] = array(
							'type' 		=> 'checkbox',
							'label' 	=> __('This is a Residential Address', 'woocommerce'),
							'class' 	=> array('my-field-class form-row-wide','update_totals_on_change'),
							'priority' 	=> 1
			...
			public function wf_admin_includes() {
				...
				if( ! class_exists('Ph_Multicarrier_Product_Settings') ) {
					include_once 'core/class-ph-multicarrier-product-settings.php';
				}
			}
			...
			public function plugin_action_links_mcp( $links ) {
				$plugin_links = array(
					...
					'<a href="https://www.pluginhive.com/knowledge-base/category/multiple-carrier-shipping-plugin-for-woocommerce/" target="_blank">' . __('Documentation', 'eha_multi_carrier_shipping') . '</a>',
					'<a href="https://www.pluginhive.com/support/" target="_blank">' . __('Support', 'eha_multi_carrier_shipping') . '</a>'
				);
