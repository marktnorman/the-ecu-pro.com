            ...
            if ( $insert_entry && ! $entry_when_unsub ) {
                ...
                if ( is_user_logged_in() && ! is_admin() ) {
                    ...
                    if ( ! is_user_logged_in() ) {
                        // new guest entry
                        $guest_email_in_cookie = isset( $_COOKIE[ 'raccookie_guest_email' ] ) && filter_var( $_COOKIE[ 'raccookie_guest_email' ] , FILTER_VALIDATE_EMAIL ) ? $_COOKIE[ 'raccookie_guest_email' ] : '' ;
                        if ( $guest_email_in_cookie ) {
                            $cart_details_compact = array ( '' , '' , '' , $guest_email_in_cookie ) ;
            ...
            if ( ! is_user_logged_in() ) {
                if ( isset( $_POST[ 'cookie_guest_email' ] ) ) {
                    ...
                    if ( filter_var( $email , FILTER_VALIDATE_EMAIL ) ) {
                        setcookie( "raccookie_guest_email" , $email , time() + 3600 , "/" ) ;
                        echo 'success' ;
                    }
                }
            }
...
//                    unset($_COOKIE['raccookie_guest_email']);
//                    setcookie('raccookie_guest_email', null, -1, '/');
                    setcookie( "raccookie_guest_email" , $visitor_mail , time() + 3600 , "/" ) ;
                    //update post meta
                    fp_rac_update_cartlist_post_meta( $update_args , $check_previous[ 0 ] ) ;
