<?php
	...
	class LQIP {
		...
		public function get( $url ) {
            //for now get rid of the getParentUrl which works bad in some situations.
            //$parent = $this->getParentUrl( $url );
		...
		private function generate( $collection ) {
			...
			if ( !empty( $collection ) && is_array( $collection ) ) {
                ...
                foreach ( $collection as $index => $item ) {
                    ...
                    $this->log('LQIP RESPONSE for ' . $lqipUrl . ': ', substr($body, 0, 300) );

					if ( $this->isPlaceholder( $body ) || $skip_request ) {
						$name = $this->getFileName( $item[ 'source' ] );
						...
						$result = [
                            ...
                            'type'       => false,
							'is_created' => false,
							'message'    => 'Received content (body) from the CDN is not a valid LQ placeholder (' . $lqipUrl . '): ' . substr($body, 0, 200),
		...
		/**
		 * Method returns parent URL if possible
		 *
		 * @param string $url
		 *
		 * @return bool|string
		...
		private function getParentUrl( $url ) {
			$parents = ShortPixelAI::get_parent_attachments( $url );
			...
			return empty( $parent ) || !$parent instanceof \WP_Post ? false : ( ShortPixelUrlTools::isValid( $parent->guid ) ? $parent->guid : false );
		}
		...
		private function replaceWithParent( $collection ) {
			...
			foreach ( $collection as &$item ) {
				...
				//for now get rid of the getParentUrl which works bad in some situations.
				//$parent_url = $this->getParentUrl( $item[ 'url' ] );
		...
		public function enqueueScripts() {
			...
			{
				wp_register_script( 'spai-lqip', $this->ctrl->plugin_url . $file, [ 'jquery', 'spai-scripts' ], $version, true );
