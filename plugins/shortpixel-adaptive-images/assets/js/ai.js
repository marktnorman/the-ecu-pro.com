...
SPAI.prototype.updateDivUrl = function(elm, hasMutationObserver, fromIntersection) {
    ...
    ShortPixelAI.updateInlineStyle(elm, wPad, hPad, ((exclusions & ShortPixelAI.EXCLUDED) == 0), false);
    ShortPixelAI.elementUpdated(elm, w);
};
...
SPAI.prototype.setupIntersectionObserverAndParse = function() {
    ...
    if(true || !navigator.platform || !/iPad|iPhone|iPod/.test(navigator.platform)) { //but NOT on iPhones, it breaks the page
...
SPAI.prototype.composeApiUrl = function(doRegister, src, w, h, specialCrop) {
    ...
    if(specialCrop) {
        var sc = "sc_" + specialCrop.clip.offsetX + "x" + specialCrop.clip.offsetY +
            ...
            ":" + specialCrop.viewport.width + "x" + specialCrop.viewport.height;
...
SPAI.prototype.containsPseudoSrc = function( pseudoSrc ) {
	return typeof pseudoSrc === 'string' ? pseudoSrc.indexOf( 'data:image/svg+xml;' ) >= 0 : false;
};
...
SPAI.prototype.updateInlineStyle = function(elm, w, h, isApi, cssData) {
    ...
    for ( var index = 0; index < pseudoSrcs.length; index++ ) {
        ...
        for (var key in cssData) {
            if(cssData[key].originalWidth != cssData[key].viewport.width || cssData[key].originalHeight != cssData[key].viewport.height) {
                specialCrop = {
        ...
        var newSrc = isApi ? ShortPixelAI.composeApiUrl(false, src, w,h, specialCrop) : src;
    ...
    //elm.css('background-image', backgroundStyle);
...
SPAI.prototype.spaiCalculateBgClipResize = function(elm) {
    ...
    if(bgImg === "none") {
        ...
        //TODO SIMON: solution for background-images which are in css blocks
    ...
    for(var i = 0, bgImgs = {}; i < bgImg.length; i++) {
        ...
        if(parsedSrc.origWidth === false || parsedSrc.origHeight === false || parsedSrc.origWidth == 1 || parsedSrc.origHeight == 1) {
            bgImgs[parsedSrc.src] = {/*style: match,*/
                width: (elmW && (style.backgroundSize == 'cover' || style.backgroundPosition == 'contain') ? elmW : false),
    ...
    function intersect(index, img, vPort) {
        ...
        if(    isRepeat(style, index, 'x')
            && (img.offsetx > 0 || img.offsetx + img.width < vPort.width) ) {
            width = img.width;
        }
        //if it's a percent leave it as it is
        else if(idxVal(style.backgroundPositionX, index).match(/^[0-9]+%$/) === null && img.offsetx < vPort.x) {
            alterCss['background-position-x'] = idxReplace(style.backgroundPositionX, index, '0%');
        }
        if(    isRepeat(style, index, 'y')
            && (img.offsety > 0 || img.offsety + img.height < vPort.height) ) {
            height = img.height;
        } else if(idxVal(style.backgroundPositionY, index).match(/^[0-9]+%$/) === null && img.offsety < vPort.y) {
            alterCss['background-position-y'] = idxReplace(style.backgroundPositionY, index, '0%');
        }
