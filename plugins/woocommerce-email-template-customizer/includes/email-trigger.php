...
class Email_Trigger {
	...
	private function __construct() {
		...
		add_action( 'woocommerce_email', array( $this, 'get_email_template_id' ) );
...
//		Email with wc_mail
	...
	public function get_email_template_id( $email_object ) {
		if ( is_array( $email_object->emails ) ) {
			foreach ( $email_object->emails as $email ) {
				...
				add_filter( 'woocommerce_email_subject_' . $email->id, array( $this, 'replace_subject' ), 10, 3 );
			}
		}
	...
	public function replace_subject( $subject, $object, $class_email ) {
		if ( $this->template_id ) {
			...
			$subject  = preg_replace( $class_email->plain_search, $class_email->plain_replace, wp_strip_all_tags( $subject ) );
	...
	public function get_template_id_no_order( $type ) {
		...
		if ( $country_code ) {
			foreach ( $posts as $post ) {
				...
				if ( is_array( $rule_countries ) && in_array( $country_code, $rule_countries ) ) {
					$temp_id = $post->ID;
					break;
				} elseif ( empty( $rule_countries ) ) {
					$temp_id = $post->ID;
					break;
				}
			}
