...
class Email_Builder {
	...
	private function __construct() {
		...
		add_action( 'admin_enqueue_scripts', array( $this, 'background_image_style' ) );
        ...
        <div id="viwec-right-sidebar">
			...
			foreach ( $boxes as $key => $data ) {
				if ( ! empty( $data['func'] ) && is_array( $data['func'] ) ) {
					...
					$object->$method( $post );
					$html = ob_get_clean();
					printf( "<div id='viwec-box-%s' class='viwec-setting-box'><div class='viwec-box-title'>%s</div>%s</div>", esc_attr( $key ), $data['title'], $html );
				}
			}
	...
	public function email_builder_box( $post ) {
		...
		wc_get_template( 'email-editor.php', array( 'admin_bar_stt' => $admin_bar_stt, 'custom_css' => $custom_css ), 'woocommerce-email-template-customizer', VIWEC_TEMPLATES );
	}
	...
	public function send_test_email() {
		...
		$message = $result ? esc_html__( 'Email was sent successfully', 'viwec-email-template-customizer' ) : esc_html__( 'Your server can\'t send email', 'viwec-email-template-customizer' );
	...
	public function add_edit_buttons( $email ) {
		...
		if ( in_array( $email->id, array_keys( $email_ids ) ) ) {
			...
			echo sprintf( "<a href='%1s' class='button '>%2s</a>", esc_url( $href ), esc_html__( 'Edit with WC Email Template Customizer', 'viwec-email-template-customizer' ) );
		} else {
			$href = admin_url( "edit.php?post_type=viwec_template&viwec_template_filter=default" );
			echo sprintf( "<a href='%1s' class='button '>%2s</a>", esc_url( $href ), esc_html__( 'Edit with WC Email Template Customizer - Default template', 'viwec-email-template-customizer' ) );
		}
		echo '</td>';
	}
