...
jQuery('#prod_test_ftp_connection').click(function(){
	...
	var use_ftp = jQuery("#pro_use_ftps").prop("checked") ? 1 : 0;
        var use_sftp = jQuery("#pro_use_sftp").prop("checked") ? 1 : 0;
        var pro_use_implicit_ftp = jQuery("#pro_use_implicit_ftp").prop("checked") ? 1 : 0;
	jQuery.ajax({
		...
		data : {
				...
				use_ftps:	use_ftp,
                                use_sftp:	use_sftp,
                                pro_use_implicit_ftp: pro_use_implicit_ftp,
                                wt_nonce:       xa_prod_piep_test_ftp.wt_nonce
				},
...
jQuery('#rev_test_ftp_connection').click(function(){
	...
	var rev_use_ftp = jQuery("#rev_use_ftps").prop("checked") ? 1 : 0;
        var use_sftp = jQuery("#rev_use_sftp").prop("checked") ? 1 : 0;
	jQuery.ajax({
		...
		data : {
				...
				use_ftps:	rev_use_ftp,
                                use_sftp:	use_sftp,
                                wt_nonce:       xa_prod_review_test_ftp.wt_nonce
				},
		success : function(response){
			jQuery('.spinner').removeClass('is-active');
