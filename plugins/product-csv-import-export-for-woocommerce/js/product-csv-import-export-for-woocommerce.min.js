jQuery(document).ready(function (a) {
    ...
    /*
    a('#pselectall').live("click", function () { jQuery .live() has been removed in version 1.9 onwards. use .on()
    ...
    */
    ...
    jQuery( "body" ).on( "click", "#pselectall", function() {  // jQuery .live() has been removed in version 1.9 onwards. use .on()
        // Iterate each checkbox
        a('#datagrid :checkbox').each(function () {
            this.checked = true;
        });
    ...
    a("select[name=export_profile]").change(function () {
        ...
        var data = {
            ...
            wt_nonce : woocommerce_product_csv_import_params.wt_nonce,
        };
        a.ajax({
...
jQuery(document).ready(function () {
    ...
    jQuery('#export_images_zip_button').click(function () {
        if (jQuery("select[name=v_export_images_zip]").val() == 1) {
            var prod_ids = jQuery("select[id='v_prods']").val();
            var offset = jQuery("input[name='offset']").val();
            var limit = jQuery("input[name='limit']").val();
            var prod_types = jQuery("select[id='v_prod_types']").val();
            var prod_categories = jQuery("select[id='v_prod_categories']").val();
            var prod_tags = jQuery("select[id='v_prod_tags']").val();
            var prod_status = jQuery("select[id='v_prod_status']").val()
            var sortcolumn = jQuery("select[id='v_prod_sort_column']").val();
            var wt_prod_sort_ord = jQuery("input[name='wt_prod_sort_ord']").val();
            var wt_nonce = woocommerce_product_csv_import_params.wt_nonce
            var path = '&prod_ids=' + prod_ids +'&offset=' + offset + '&limit=' + limit + '&prod_types=' + prod_types + '&prod_categories=' + prod_categories + '&prod_tags=' + prod_tags + '&prod_status=' + prod_status + '&sortcolumn=' + sortcolumn + '&wt_prod_sort_ord=' + wt_prod_sort_ord+ '&wt_nonce=' + wt_nonce ;
             window.open(woocommerce_product_csv_import_params.siteurl + '?page=wf_woocommerce_csv_im_ex&action=export_images'+path, '_blank')
         }
    });
    ...
    jQuery("#v_save_export_mapping").click(function () {  
        ...
        var specific_metas = jQuery("#v_specifi_metas").val();
        //alert(specific_metas);
        var data = {
            action: 'product_csv_export_mapping_save',
            profile_name: profile_name,
            columns: columns,
            columns_name: columns_name,
            specific_meta: specific_metas,
            wt_nonce: woocommerce_product_csv_import_params.wt_nonce
