...
//    var $processed_terms = array();
    ...
    public function __construct() {
        ...
        $this->import_page = 'wt_woocommerce_product_csv';
    ...
    public function dispatch() {
        ...
        if (!$this->new_prod_status) {
            $this->new_prod_status = !empty($_POST['new_prod_status']) ? sanitize_text_field($_POST['new_prod_status']) : null;
        }
...
// Get CSV positions
                    if (( $handle = @fopen($file, "r") ) !== FALSE) {
                        $csv_delimiter = self::wt_get_csv_delimiter($this->delimiter); //($this->delimiter=='tab'?"\t":$this->delimiter);
...
//                $this->processed_terms = isset($_POST['processed_terms']) ? array_map('intval',$_POST['processed_terms']) : array();
                ...
                $this->delete_products = isset($_POST['delete_products']) ? absint($_POST['delete_products']) : 0;
            
                $file = isset($_POST['file']) ? stripslashes($_POST['file']) : ''; 
                $this->id=isset($_POST['file_id']) ? absint($_POST['file_id']) : 0;
    ...
    public function xml_import($file) {        
        ...
        if ($xml) {
            ...
            $fp = fopen($file, 'w');
            $this->delimiter = ','; //  set default delimter ',' for import 
        ...
        $file = apply_filters('wt_product_import_xml_file_before', $file);
        return $file;
    }
    ...
    public function import_options() {
        ...
        $file = apply_filters('wt_product_import_file_before', $file);
        ...
        if (( $handle = @fopen($file, "r") ) !== FALSE) {
            ...
            while (( $postmeta = ($this->decentFgetcsv) ? fgetcsv($handle, 0, $csv_delimiter, '"', '"') : fgetcsv($handle, 0, $csv_delimiter, '"') ) !== FALSE) {
                foreach ($header as $key => $heading) {
                    ...
                    $row[$s_heading] = ( isset($postmeta[$key]) ) ? wf_piep_helper::format_data_from_csv($postmeta[$key], $use_mb) : '';
    ...
    public function import() {
        ...
        foreach ($this->parsed_data as $key => &$item) {
            ...
            if (!is_wp_error($product)){
                try{
                    $this->process_product($product, $this->new_prod_status, $item);
                } catch (Exception $ex) {
    ...
    public function save_product_id_from_csv() {
        global $product_to_be_deleted;        
            ...
            if($product_to_be_deleted_new_array){
    ...
    public function delete_products_not_in_csv() {
        ...
        if (!defined('EMPTY_TRASH_DAYS') || ( EMPTY_TRASH_DAYS == 0 || EMPTY_TRASH_DAYS == '' || EMPTY_TRASH_DAYS == null )) {
            ...
            if(!empty($product_to_be_deleted_new_array)){
                $query = "SELECT ID FROM $wpdb->posts WHERE post_type IN ('product', 'product_variation') AND post_status != 'trash'";
                ...
                if (!empty($product_to_be_deleted_final)) {
                    ...
                    $product_to_be_deleted_final = apply_filters('wt_product_to_be_deleted_array',$product_to_be_deleted_final); // product ids which is going to delete
...
//        unset($import_data);
    ...
    public function handle_upload() {
        ...
        if (empty($_POST['file_url']) && (!empty($_FILES['import']['name']) )) {
            ...
            if (isset($file['error'])) {
                echo '<p><strong>' . __('Sorry, there has been an error on uploading file.', 'wf_csv_import_export') . '</strong><br />';
                echo esc_html($file['error']) . '</p>';
                return false;
            }
			 $file = apply_filters('wt_handle_uploaded_file', $file);
            $this->id = (int) $file['id'];
            return true;
    ...
    public function get_data_from_url($url) {
        ...
        if($file_path = apply_filters('parse_data_in_url', $default=false, $url , $local_file)){
                
        } elseif($file_path = $this->get_data_from_url_method_1($url,$local_file))
        {
            //return $file_path;
...
//            return esc_attr(str_replace(ABSPATH, "", $local_file));
     ...
     */
    public function process_product($post, $new_product_status = null, $parsed_item) {
        global $product_to_be_deleted;
        ...
        if (!empty($post['sku']) && empty($post['wpml'])) {
            $id_exist = wf_piep_helper::wt_get_product_id_by_sku($post['sku']);
            if($id_exist == $processing_product_id || $processing_product_sku == $post['sku'] || empty ($id_exist)){
                $processing_product_sku = $post['sku'];
            }else{
                if ($this->delete_products == 1 && !empty($id_exist)) {
                   $product_to_be_deleted[] =$id_exist;
                }

                $usr_msg = 'Invalid or duplicated SKU.';
                $processing_product_title = sprintf('<a target="_bank" href="%s">%s</a>', get_edit_post_link($id_exist), get_the_title($id_exist));
                $this->add_import_result('skipped', __($usr_msg, 'wf_csv_import_export'), $id_exist, $processing_product_title, $post['sku']);
        ...
        if (!empty($post['sku']) && (isset($post['wpml']) && !empty($post['wpml']))) {
            $processing_product_sku = $post['sku']; 
        }
        ...
        if (!$merging) {
            ...
            if (!empty($processing_product_id) && (in_array($is_post_type_product, array('product','product_variation')))) {
                ...
                $processing_product_title = sprintf('<a target="_bank" href="%s">%s</a>', get_edit_post_link($processing_product_id), $processing_product_title);
...
//            $existing_product = (isset($processing_product_sku) && !empty($processing_product_sku) ? wc_get_product_id_by_sku($processing_product_sku) : ''); //$this->product_exists($processing_product_title, $processing_product_sku, $post['post_name']);
            ...
            if ($existing_product) {
                ...
                $processing_product_title = sprintf('<a target="_bank" href="%s">%s</a>', get_edit_post_link($existing_product), $processing_product_title);
            ...
            if ($processing_product_id && is_string(get_post_status($processing_product_id))) {
                $processing_product_title = sprintf('<a target="_bank" href="%s">%s</a>', get_edit_post_link($processing_product_id), get_the_title($processing_product_id));
                $this->add_import_result('skipped', __('Importing product(ID) conflicts with an existing post.', 'wf_csv_import_export'), $processing_product_id, $processing_product_title , '');
        ...
        if (!empty($post['tax:product_type']) && $post['tax:product_type'] == 'product_variation') {
            $post['post_type'] = 'product_variation';
        }elseif($merging && empty($post['tax:product_type'])){
            $post['post_type'] = $processing_product ? $processing_product->post_type : $post['post_type'];            
        }
        unset($processing_product);
...
//        if ($merging && $processing_product_id && !empty($is_post_type_product) && ($is_post_type_product !== $post['post_type'] )) 
          if ($merging && $processing_product_id && !empty($is_post_type_product) && (!in_array($is_post_type_product, array('product','product_variation')))){ //OCSEIPFW-222
            $processing_product_title = sprintf('<a target="_bank" href="%s">%s</a>', get_edit_post_link($processing_product_id), get_the_title($processing_product_id));
        ...
        if (!empty($post['postmeta']) && is_array($post['postmeta'])) {
            ...
            if(isset($post['tax:product_type']) && !empty($post['tax:product_type'])){  // please double check this change 
                $product_type = $post['tax:product_type'];
            }            
            ...
            foreach ($post['postmeta'] as $meta) {
                ...
                if ($key) {
                    ...
                    $meta_key = ( ( stristr($key, 'attribute') != false ) ? strtolower($key) : $key);

                    $meta_value = (in_array($key, $escape_serialize_keys) ? json_decode($meta['value'],TRUE) : maybe_unserialize($meta['value']));
...
//        }
        ...
        if ($merging) {
            $processing_product_title = sprintf('<a target="_bank" href="%s">%s</a>', get_edit_post_link($post_id), $processing_product_title);
        ...
        do_action('wf_refresh_after_product_import',$processing_product_object ,$post ,$parsed_item); // hook for forcefully refresh product
        unset($post, $processing_product_object, $parsed_item);
    }
...
//                    $this->attachments[] = $attachment_id;
    ...
    public function fetch_remote_file($url, $post) {
        ...
        if (!$wp_filetype['type']) {
            ...
            if (isset($headers['content-disposition']) && strstr($headers['content-disposition'], 'filename=')) {
                ...
                $img_name = (isset($image_name[0]) && !empty($image_name[0])) ? $image_name[0]:'image';
                $file_name = $img_name . '.' . str_replace('image/', '', $headers['content-type']);
            }
    ...
    private function handle_ftp() { 
        ...
        $ftp_server = !empty($_POST['pro_ftp_server']) ? sanitize_text_field($_POST['pro_ftp_server']) : '';
        $ftp_server_path = !empty($_POST['pro_ftp_server_path']) ? sanitize_text_field($_POST['pro_ftp_server_path']) : '';
        $ftp_user = !empty($_POST['pro_ftp_user']) ? wp_unslash($_POST['pro_ftp_user']) : '';
        $ftp_password = !empty($_POST['pro_ftp_password']) ? wp_unslash($_POST['pro_ftp_password']) : '';
        $ftp_port = !empty($_POST['pro_ftp_port']) ? absint($_POST['pro_ftp_port']) : 21;
        ...
        $pro_use_implicit_ftp = !empty($_POST['pro_use_implicit_ftp']) ? true : false;
        ...
        if (class_exists('class_wf_sftp_import_export') && $pro_use_sftp == 1) {
            ...
            if($pro_use_implicit_ftp){
                if (self::implicit_ftp_file_download($server_file, ABSPATH . $local_file, $settings['pro_ftp_server'], $settings['pro_ftp_user'], $settings['pro_ftp_password'])) {
                    $error_message = "";
                    $success = true;
                } else {
                    $error_message = "Failed to Download Specified file in FTP Server File Path.<br/><br/><b>Possible Reasons</b><br/><b>1.</b> File path may be invalid.<br/><b>2.</b> Maybe File / Folder Permission missing for specified file or folder in path.<br/><b>3.</b> Write permission may be missing for file <b>plugins/product-csv-import-export-for-woocommerce/temp-import.csv</b> .\n";
        ...
        if ($success) {
            ...
            die(__($error_message,'wf_csv_import_export'));
        }
        return true;
    }
    ...
    public function greet() {
        $action = 'admin.php?import=wt_woocommerce_product_csv&amp;step=1';
    ...
    public static function wt_get_csv_delimiter($delemiter=','){
     ...
     public static function implicit_ftp_file_download($remote, $local = null, $server, $username, $password) {
