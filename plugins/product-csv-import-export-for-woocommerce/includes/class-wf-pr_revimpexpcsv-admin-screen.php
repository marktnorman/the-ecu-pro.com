...
class WF_ProdReviewImpExpCsv_Admin_Screen {
    ...
    public function custom_comment_column_data($column, $comment_ID) {
        if ('comment_export_to_csv' == $column) {
            ...
            $url_general = wp_nonce_url(admin_url('admin-ajax.php?action=comment_export_to_csv_single&comment_ID=' . $comment_ID), WF_PR_REV_IMP_EXP_ID);
    ...
    public function admin_scripts() {
        ...
        if (in_array($screen->id, $allowed_creen_id)) {
            ...
            wp_enqueue_style('woocommerce-product-csv-importer1', plugins_url(basename(plugin_dir_path(WF_PrRevImpExpCsv_FILE)) . '/styles/wf-style.css', basename(__FILE__)), '', WT_PROD_IMP_EXP_VER, 'screen');
            wp_enqueue_style('woocommerce-product-csv-importer3', plugins_url(basename(plugin_dir_path(WF_PrRevImpExpCsv_FILE)) . '/styles/jquery-ui.css', basename(__FILE__)), '', WT_PROD_IMP_EXP_VER, 'screen');

            wp_enqueue_script('woocommerce-product-csv-importer2', plugins_url(basename(plugin_dir_path(WF_PrRevImpExpCsv_FILE)) . '/js/product-rev-csv-import-export-for-woocommerce.min.js', basename(__FILE__)), '', WT_PROD_IMP_EXP_VER, 'screen');
