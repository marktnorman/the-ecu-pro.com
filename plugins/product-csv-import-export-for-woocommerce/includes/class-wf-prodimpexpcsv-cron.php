...
class WF_ProdImpExpCsv_Cron {
    ...
    public function wf_scheduled_export_products() {
        ...
        $export_limit                = apply_filters('wt_product_export_limit', $export_limit);
        $current_offset             = apply_filters('wt_product_export_offset', $current_offset);
        
        if(!empty($current_offset)){
            $_POST['offset'] = $current_offset;
        }
        if(!empty($export_limit)){
            $_POST['limit'] = $export_limit;
        }
        ...
        if (isset($this->settings['pro_auto_export_delimiter'])) {
            $_POST['delimiter'] = $this->settings['pro_auto_export_delimiter'];
        }
