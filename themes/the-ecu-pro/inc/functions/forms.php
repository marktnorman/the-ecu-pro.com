...
function insertNewComment($name, $wp_comment_content) {
  ...
  if (wp_new_comment($comment_data)) {
    if (!empty($_POST['redirect_url'])) {
      wp_redirect($_POST['redirect_url']);
      exit;
    }
    $tecup_form_success = true;
    return true;
  }
  return false;
}
...
function save_contact_message() {
  ...
  if (filter_var($_POST['email'], FILTER_VALIDATE_EMAIL) && !empty($_POST['cname']) && !empty($_POST['vehicle']) && !empty($_POST['message'])) {
    $name = $_POST['cname'];
